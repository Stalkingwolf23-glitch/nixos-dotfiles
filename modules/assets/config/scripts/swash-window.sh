...
timeout_seconds="${SWASH_SCREENSHOT_TIMEOUT:-15}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
BUILD_DIR="${BUILD_DIR:-${ROOT_DIR}/build}"
clipboard_mime="${SWASH_SCREENSHOT_CLIPBOARD_MIME:-image/png}"
clipboard_name="${SWASH_SCREENSHOT_NAME:-Screenshot from $(date '+%Y-%m-%d %H-%M-%S').png}"
...
resolve_swash_bin() {
  ...
  if [[ -x "${BUILD_DIR}/src/swash" ]]; then
    printf '%s\n' "${BUILD_DIR}/src/swash"
    return 0
  fi

  printf 'could not find swash. Install it on PATH or build it in %s.\n' "${BUILD_DIR}" >&2
  exit 1
}
...
open_clipboard_image() {
  ...
  setsid -f bash -c 'wl-paste --type "$1" | "$2" --stdin --name "$3"' bash "$mime" "$swash_bin" "$clipboard_name" >/dev/null 2>&1
}
...
niri msg action screenshot-window >/dev/null
...
if [[ -n "$screenshot_path" ]]; then
  setsid -f "$swash_bin" "$screenshot_path" >/dev/null 2>&1
